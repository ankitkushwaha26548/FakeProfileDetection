...
const logPostActivity = (userId, type, postId) => {
  logActivity(userId, type, postId);
  runDetection(userId).catch(err => console.error(`Detection failed (${type}):`, err.message));
};

export const createPost = async (req, res) => {
  try {
    const post = await Post.create({ user: req.user._id, content: req.body.content });
    logPostActivity(req.user._id, 'POST', post._id);
...
export const likePost = async (req, res) => {
  try {
    ...
    if (!post.likes.includes(req.user._id)) {
      ...
      await post.save();
      logPostActivity(req.user._id, 'LIKE_POST', post._id);
    }
    res.json(post);
  } catch (error) {
    res.status(500).json({ message: error.message });
...
export const commentOnPost = async (req, res) => {
  try {
    ...
    await post.save();
    logPostActivity(req.user._id, 'COMMENT', post._id);
    res.json(post);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
